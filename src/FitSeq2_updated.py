...
class FitSeq2:
    ...
    def traj_LL_from_n(self, n0, s):
        ...
        print("Debug: M_matrix before solve:", M_matrix)
        print("Debug: Determinant of M_matrix:", np.linalg.det(M_matrix))
        print("Debug: Rank of M_matrix:", np.linalg.matrix_rank(M_matrix))
        if np.linalg.det(M_matrix) == 0 or np.linalg.matrix_rank(M_matrix) < M_matrix.shape[0]:
            print("Warning: M_matrix is singular or nearly singular. Using pseudo-inverse.")
            nu_vec = np.dot(np.linalg.pinv(M_matrix), b_vec)
        else:
